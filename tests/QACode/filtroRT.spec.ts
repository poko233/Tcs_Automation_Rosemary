...
//Descripción:
// La automatización busca validar que al aplicar el filtro de nombres en el rango **R–T**,
// los resultados mostrados correspondan únicamente a Fixers cuyos nombres comienzan con esas letras.
/////////

test("Seleccionar y validar filtro R–T", async ({ page }) => {
  ...
  // Paso 5: Activar checkbox R–T usando accesible name
  await page.getByRole("checkbox", { name: "De (R-T) (16)" }).click();
  await esperar(5);
...
// Validar que los nombres comiencen con R–T
async function validarNombres(page: Page): Promise<void> {
  ...
  for (const texto of nombres) {
    ...
    if (nombre.length > 0) {
      ...
      if (inicial >= "R" && inicial <= "T") {
        console.log(`Nombre válido: ${nombre}`);
      } else {
        console.log(`Nombre fuera del rango R–T: ${nombre}`);
        todosValidos = false;
      }
    }
  }
  ...
  expect(
    todosValidos,
    "Se encontraron nombres fuera del rango permitido (R–T)."
  ).toBe(true);
}
